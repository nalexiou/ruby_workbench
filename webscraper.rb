...
LIST_URL = "#{BASE_WIKIPEDIA_URL}/wiki/Academy_Award_for_Best_Picture"
...
File.open("output.txt", 'w') do |file| 
	...
	rows.each do |row|
		 ...
		 hrefs.each do |href|
		 	...
		 	p budget_usdollars_match = budget.match(/\$([\d,.]+)(?:[\–\-])?(\d)?\s?(million)?/i)
		 	...
		 	if !budget_usdollars_match.nil?
		 		...
		 		p budget_total.round.to_s.reverse.scan(/\d{3}|.+/).join(",").reverse
				p budget_count += 1
		 	end
